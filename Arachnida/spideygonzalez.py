    ...
    for img in soup.find_all("img"):
        ...
        if src and not src.startswith("data:"):
            ...
            if filename in downloaded_files:
                ...
                continue
            print(f"\033[1;32mDownloading {filename}\033[0m \033[1;33mfrom {src}\033[0m in {folder}")
...
# def scrape_images(url, folder, recursive=False, level=1, downloaded_files=set()):
#     if level < 1:
#         return
#     if not os.path.exists(folder):
#         os.makedirs(folder)
#     download_images(url, folder, level, downloaded_files) 
#     if recursive:
#         page = requests.get(url)
#         soup = BeautifulSoup(page.content, "html.parser")
#         for link in soup.find_all("a"):
